(ns guardops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (GuardOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the GuardOpsGovernor (:govern)
  ...
  human operator/security supervisor. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected).

  This actor NEVER dispatches a guard to an incident and NEVER
  authorizes any physical intervention (use of force, detention,
  arrest) -- see `guardops.governor` for the permanent, un-overridable
  block on that territory. Every commit here is a scheduling/logging/
  reporting record about a security operation a human is already
  directing, never a directive to act."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [guardops.advisor :as advisor]
            [guardops.governor :as governor]
...
(defn build
  ...
  `guardops.store/Store`).
  opts:
    :advisor      -- a `guardops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; GuardOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; GuardOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :assignment-id (:assignment-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :assignment-id (:assignment-id request) :by (:by approval)}]}
            {:disposition :hold
